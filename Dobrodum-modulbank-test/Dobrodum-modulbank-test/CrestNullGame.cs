...
namespace Dobrodum_modulbank_test
{
    ...
    public class CrestNullGame
    {
        ...
        public void NextMove(uint x, uint y, char symbol)
        {
            ...
            if (Round % 3 == 0 && new Random().Next(10) == 0)
                if (symbol == 'X')
                    symbol = '0';
                else
                    symbol = 'X';
        ...
        private void CheckWinSpare(char[,] newFieldState, uint x, uint y, char symbol)
        {
            ...
            for (int n = 0; n < FieldSize; n++)
                for (int m = 0; m < FieldSize; m++)
                    neutralCount += newFieldState[n, m] == '-' ? (uint)1 : 0;
            if (PercentageOfOccupiedCells <= 100 - neutralCount * FieldSize * 2 / 100 )
